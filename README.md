...
- **Gaps in the Shelly's history** (for example while the Shelly was off) are skipped: the cursor jumps to the next stored record. Their energy cannot be recovered, but the gaps are reported: the diagnostic sensor has the attributes `gaps`, `missing_minutes`, `last_gap_start` and `last_gap_end`, every gap is logged, and a gap of 10 minutes or more raises a notice under **Settings → System → Repairs** (which you can ignore).
...
- Shelly firmware 2.x has a brute-force protection that answers HTTP 429 (Too Many Requests) to a steady stream of unauthenticated requests, and firmware 2.0.0 does so now and then even for a single client (reportedly fixed in 2.0.1, first released as a beta). To stay clear of it, the integration reuses the digest challenge between calls instead of provoking a new one with every request. If the Shelly still answers with 429, the request is repeated up to three times (following a `Retry-After` header if the Shelly sends one) before the poll counts as failed. A failed poll makes the sensors unavailable until the next successful one; no data is lost, because the read position only moves after a successful read.
...
- The Shelly stamps its records with its own clock. At start and then once an hour the integration compares that clock with Home Assistant's (one extra request); if it is more than two minutes off or not set at all (no time server reachable), a notice appears under **Settings → System → Repairs**, because the energy would be assigned to the wrong hours. The difference is shown as the attribute `clock_offset_seconds` of the diagnostic sensor.
