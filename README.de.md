...
- Der Shelly versieht seine Datensätze mit seiner eigenen Uhr. Beim Start und danach stündlich vergleicht die Integration diese Uhr mit der von Home Assistant (eine zusätzliche Anfrage); weicht sie um mehr als zwei Minuten ab oder ist gar nicht gesetzt (kein Zeitserver erreichbar), erscheint ein Hinweis unter **Einstellungen → System → Reparaturen**, weil die Energie sonst den falschen Stunden zugeordnet würde. Die Abweichung steht als Attribut `clock_offset_seconds` am Diagnose-Sensor.
